...
- **B-2 PrConversationTab** — ✅ 완료(commit e995545). PrDetailModal 의 Conversation 탭(코멘트 스레드 + 새 코멘트 + code suggestion form + 리뷰 제출 + AI 리뷰)을 PrConversationTab.vue 로 분리. **usePrMutations 를 usePrConversationMutations(addComment/review/suggestion) + usePrActionMutations(merge/close/reopen)로 분할**, usePrMutations 는 둘을 compose 하는 thin wrapper 로 유지(useMutation 호출 순서 0~5 보존 → 기존 테스트 8건 green). footer 는 action, conversation 은 PrConversationTab 이 소유, detail/comments 는 prop·deduped query, Files 탭 "+" prefill 은 defineExpose. 검증: typecheck 0 / lint 0 / usePrMutations.test 8 / vitest 929. (PR 모달은 ui:sweep 미포함 → 테스트 검증)
- **B-3 CommitGraphRow** — ✅ 완료(commit f17eeed). commit row(6컬럼 switch + ref pills + avatar)를 DOM-identical 분리. **ref-visibility state 가 module 싱글톤(soloRef)+vue-query(hiddenRefs)라 부모 sticky overlay 와 공유**되는 것을 활용 — 부모 단일 인스턴스에서 visibleRef/soloRef/refPillClass/toggle/hide 를 unwrap 해 prop 전달(반응성 보존, per-row 재인스턴스 회피; "desync/18-prop" 초기 우려는 shared-state 확인으로 해소). bodyFirstLine/authorInitial/authorAvatarBg 직접 import, shownRefs 등 자식 로컬. **v-memo 는 outer wrapper div DOM 재설계 필요라 분리(별도 sprint)** — 본 추출은 DOM-identical. 검증: typecheck 0 / lint 0 / vitest 929 / **ui:sweep maintab-00-graph 정상**(ref pill 렌더 + console 0). CommitGraph total 664→524, script 231→214(Layer A 잔존).
  - 적대적 quality 리뷰 HIGH 2건(`computed`/`CommitRefPill` 미import) = **auto-import 미고려 FALSE POSITIVE** (AutoImport({imports:['vue']}) + components.d.ts + typecheck 통과 + 런타임 console 0 로 입증).
