...
| **C7** 옵션 노출 | ◐ 대부분 | ✅ A-3·B4-07·UXF-10·B4-04·A-11·A-17·A-2 / ⏳ A-19 |
| **C8** Settings UX | ◐ 대부분 | ✅ R2-S1·R2-S2·R6-002·R2-R1·R2-R2 / ⏳ R2-WR1·R2-GH1 |
| **C9** i18n·일관성 | ✅ 완료 | R6-003·A-7·A-23 / R5-002·UXF-34 = cosmetic catch-all |
| **C10** colorblind·empty | ◐ 대부분 | ✅ UXF-30·B4-10 / A-8 기구현 / ⏳ A-20·UXF-27 |

### 완료 (HIGH 2 + MED 대부분 + LOW 다수, ~80건 / 22 commit)

C1~C6 전체 + C7~C10 대부분. HIGH 2건(UXF-01 merge marker stage 차단, UXF-05 IPC timeout 안내) 모두 해소.
신규 인프라: `useAppExitGuard`, `chooseDialog`+`ChooseDialog.vue`, `progress`/`errorBoundary`/`notification`/`graph`/`bisect` i18n 네임스페이스.

### 잔여 작업 (4건 — 별도 sprint 권고)

| 항목 | 사유 | effort |
| --- | --- | --- |
| A-19 graph author/branch 필터 | 필터 칩 UI + 클라이언트 필터 또는 Rust query 파라미터 — 단독 feature 규모 | L |
| A-20 graph lane colorblind | canvas renderer hover 레인 레이블/패턴 변형 — canvas 인터랙션 신규 | M |
| R2-WR1 Settings/Repositories 반응형 | 좁은 윈도우 대응 레이아웃 (breakpoint + drawer) | M |
| R2-GH1 / UXF-27 | githooks repo 안내 버튼(LOW) / status flat→tree 자동전환(UX 결정 필요) | S |
