...
## 구현 진행 상황 (2026-05-21 — `/goal` 개선 sprint)

> backlog 를 클러스터 단위로 구현. commit `b20e2bf`~`a440b3d` (8 commit). 각 단계 typecheck + lint + vitest 912 PASS.

| 클러스터 | 상태 | commit | 비고 |
| --- | --- | --- | --- |
| **C1** 파괴적 액션 confirm | ✅ 완료 | `b20e2bf` | UXF-01·R2-ST1·R2-ST2·R2-WT1·R2-WT3·A-16·A-18·B4-02·UXF-25 |
| **C2** 비동기 작업 상태 | ✅ 완료 | `9280de4` | UXF-05·UXF-06·UXF-08·UXF-21·B4-09·R5-005 |
| **C3** 키보드 접근성 | ✅ 완료 | `658b3f2` | UXF-03·04·13·19·20·24, A-6·A-9·A-21, B4-03 |
| **C4** 에러 메시지 | ✅ 완료 | `c8ef28d` | UXF-14·UXF-18·A-24·A-10 / UXF-31·R2-WT3 기구현 |
| **C5** 멀티스텝 가시성 | ✅ 완료 | `8699e76` | UXF-15·R2-D1 / UXF-02 REJECTED |
| **C6** unsaved 보존 | ✅ 완료 | `4044d44` | UXF-11·UXF-12 / UXF-16·R2-FR3 해당없음 |
| **C7** 옵션 노출 | ◐ 부분 | `24e9155` | ✅ A-3·B4-07 / ⏳ UXF-10·B4-04·A-2·A-11·A-17·A-19 |
| **C8** Settings UX | ⏳ 미착수 | — | R2-S1·S2·R6-002·WR1·R-1·R-2·GH1 |
| **C9** i18n·일관성 | ◐ 부분 | `a440b3d` | ✅ R6-003·A-7 / ⏳ A-23·R5-002·UXF-34 |
| **C10** colorblind·empty | ◐ 부분 | `a440b3d` | ✅ UXF-30 / A-8 기구현 / ⏳ A-20·B4-10·UXF-27 |

### 완료 (HIGH 2 + MED 다수, ~58건)

C1~C6 전체 + C7/C9/C10 일부. HIGH 2건(UXF-01 merge marker stage 차단, UXF-05 IPC timeout 안내) 모두 해소.

### 잔여 작업 (별도 sprint 권고 — Rust IPC / 신규 UI infra 필요)

| 항목 | 사유 | effort |
| --- | --- | --- |
| UXF-10 branch drag action sheet | multi-option dialog 컴포넌트 신규 (confirm/prompt 형제) | M |
| B4-04 push force-with-lease/tags | split-button 드롭다운 UI | M |
| A-2 PR head 브랜치 checkout | fetch + checkout 플로우 (cross-fork `pull/N/head` 포함) | M |
| A-11 tag target SHA | Rust `create_tag` IPC 시그니처 변경 | M |
| A-17 bisect good/bad ref | Rust `bisect_start` IPC 시그니처 변경 | M |
| A-19 graph author/branch 필터 | 필터 칩 UI + 클라이언트 필터 또는 Rust query 파라미터 | L |
| C8 Settings UX 7건 | settings shell 공통 개선 (save 피드백·nav 검색·reset·반응형·repo remove/alias) | M~L |
| A-20 graph lane colorblind | canvas renderer hover 레이블/패턴 | M |
| B4-10 unborn repo CTA | 빈 그래프 empty state + 첫 커밋 안내 | S |
| A-23·R5-002·UXF-34 | 단축키 표기 통일·copy 문구·버튼 일관성 (cosmetic) | S |
| UXF-27 status flat→tree 기본 | 자동 view 전환 (UX 결정 필요 — 사용자 선호 침범 가능) | S, 결정 필요 |
