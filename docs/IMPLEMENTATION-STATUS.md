...
| **i18n 기초 인프라** | ✅ **활성화** (vue-i18n **11.4.2** + **ko/en 1327 leaf 완전 대칭 SoT** + JSON top-level dup-key raw regex 가드 정착) | **SoT 명령**: `node scripts/i18n-leaf-count.mjs` (출력 `ko=en=1327, symmetry OK`). **UltraPlan v0.4 LLM-test (2026-05-16)**: `a11y.ariaLabel.*` namespace 신규 (16 key × ko/en) — aria-label 한글 하드코딩 16건 회수 위한 SoT. 본 sprint 마이그 5건 (CommitDetailSidebar/CommitGraph/CommitMessageInput/settings) + 잔존 11건 (useI18n import 미사용 6 컴포넌트) 후속 sprint. 1311 → 1327 (+16). **c89-B dup-key 회복** (commit `96d0be5` + 가드 `70e6ea9`): `gitkrakenImport` namespace ko/en 양쪽 중복 → 13 leaf silent drop, JSON.parse last-write-wins 메커니즘으로 i18n leaf count 우회. raw regex `^  \"([^\"]+)\":` 사후 검증 추가로 재발 차단. 누적 회복: 1298→1311 leaf. **c74** 폰트 pretendard → @fontsource Roboto Flex + Noto Sans KR + i18n 1201→1250. **c43~c48** 누적 +200 키. c40: vue-i18n ^9→^11. c38 +64 키. |
