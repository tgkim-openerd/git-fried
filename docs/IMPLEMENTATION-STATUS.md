...
작성: 2026-04-30 / 갱신: 2026-05-04 Sprint c31 22 + c32 3 + c33 14 + c34 5 + c35 6 + c36 4 + c37 11 + c38 5 + c39 1 + **c40 11** = 82 commit 누적 후 / 트리거: c40 `/analyze` 6 Recommendations 자율 + god 5 후속 분해 (commands.rs Workspace 시범 + vue-i18n ^9→^11 + 5 god composable 추출 -443 LOC + e2e stash/worktree spec)
...
| **Tauri IPC** | ✅ **168 등록** (자동 카탈로그 SoT — `docs/api/tauri-commands.md`) | c38 +6: `restore_paths` / `push_stash_staged` / `stash_to_branch` / `range_diff` / `open_path_in_explorer` + `clone_repo` filter 옵션 확장. c39: ipc/ 9→12 파일 (v02_commands.rs 분해 bisect/lfs/rebase). **c40: 13 파일** (commands.rs Workspace 4 commands → workspace_commands.rs 시범 분해 — lib.rs path 갱신 + ipc/mod.rs `pub use`). 자동 카탈로그 `docs/api/tauri-commands.md` (`bun scripts/generate-tauri-commands-index.mjs`) |
...
| **테스트** | ✅ **vitest 60 / 660 tests** / cargo test 186 / **E2E 8 spec / 신규 6 tests** / bench compile | c40: e2e/stash.spec.ts (3) + e2e/worktree.spec.ts (3) — devMock 3 STASHES + 4 WORKTREES 의 mini-section 노출 검증, 2회 연속 재현 ✓. c38 cargo +16 (E1 +6 / E3 +2 / E4 +3 / E2 +5). c39 cargo +7: forge/model.rs +4 (serde camelCase 가드) / ipc/bisect_commands.rs +3. Sprint c37 +3 files (+48 tests) |
...
| **i18n 기초 인프라** | ✅ **활성화** (vue-i18n **11.4.0** + **614 키 / 25+ 컴포넌트 활용 / ko·en 완전 대칭 검증**) | c40: vue-i18n ^9 → ^11 업그레이드 (createI18n + legacy:false 호환) + StashPanel/RepoSpecificForm 한글 8건 마이그 → +8 키 (stash 5 + repoConfig 3 신규 namespace). c38 +64 키: restore 14 / stash 10 (smart) / clone 24 / rangeDiff·compare 10 / worktree 6. c39: drift 0 / **`missingWarn`/`fallbackWarn` DEV 모드 ON** |
...
| **God component 분리** | ✅ **27 컴포넌트 / -2,122 LOC (-37%) + c40 +6 composable (-496 LOC)** | Sprint c37 +4: useGraphWidth + useGraphSelection + useStatusSelection + useStashPopMutation. c39 +3: useMaintenanceActions / useThemeIO / useLaunchpadRows. **c40 +6**: useToolbarSyncMutations (GitKrakenToolbar 495→437) / useBranchDragDrop (BranchPanel 518→419) / useLaunchpadActions (launchpad 570→515) / usePrMutations (PrDetailModal 589→466) / useGraphCanvasRenderer (CommitGraph 737→623) / useStatusTreeRows (StatusPanel 715→668). AI composable 5 표준 (c35). 500+ LOC 잔여 1개: settings 613 (script 147 / template 463 — script 영역 이미 작아 template sub-component 분해는 별도 sprint) |
