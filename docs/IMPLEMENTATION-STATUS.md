...
작성: 2026-04-30 / 갱신: 2026-05-04 Sprint c31 22 + c32 3 + c33 14 + c34 5 + c35 6 + c36 4 + c37 11 + c38 5 + c39 1 + c40 11 + **c40 후속 4** = 86 commit 누적 후 / 트리거: c40 후속 4 (cargo 환경 복구 → commands.rs 3 도메인 분해 -442 LOC + settings template 분해 -143 LOC + e2e repositories spec)
...
| **Tauri IPC** | ✅ **168 등록** (자동 카탈로그 SoT — `docs/api/tauri-commands.md`) | c38 +6: `restore_paths` / `push_stash_staged` / `stash_to_branch` / `range_diff` / `open_path_in_explorer` + `clone_repo` filter. c39: ipc/ 9→12 파일 (v02 → bisect/lfs/rebase). c40: 13 (workspace_commands 시범). **c40 후속: 16 파일** (commands.rs 1350 → 908 LOC, branch_commands 8 / stash_commands 10 / repo_commands 5 추가 분해). cargo check + cargo test 185/185 검증 ✓ |
...
| **테스트** | ✅ **vitest 60 / 660 tests** / cargo test **185/185** / **E2E 9 spec / 신규 9 tests** / bench compile | c40 후속: e2e/repositories.spec.ts (3 — Browse/Clone/Fetch All 헤더 + Modal a11y mount). c40: e2e/stash.spec.ts (3) + e2e/worktree.spec.ts (3) 2회 재현 ✓. cargo: c40 후속 commands.rs 분해 후 검증 (185 passed). c38 cargo +16 / c39 +7. Sprint c37 +3 files (+48 tests) |
...
| **God component 분리** | ✅ **29 컴포넌트 / -2,265 LOC (-39%) + c40 후속 +2 sub-component (-143 LOC)** | Sprint c37 +4: useGraphWidth + useGraphSelection + useStatusSelection + useStashPopMutation. c39 +3: useMaintenanceActions / useThemeIO / useLaunchpadRows. c40 +6: useToolbarSyncMutations / useBranchDragDrop / useLaunchpadActions / usePrMutations / useGraphCanvasRenderer / useStatusTreeRows. **c40 후속 +2 sub-component**: SettingsMaintenance.vue (102 LOC) + SettingsPluginIntegration.vue (67 LOC) → settings 613 → 470 LOC. **god ≥400 LOC 잔여 0** — 모든 god 분해 완료 |
