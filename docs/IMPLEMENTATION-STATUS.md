...
> **목적**: 38개 plan 문서 + CHANGELOG Unreleased + lib.rs invoke_handler + **8 SQLite migrations** + **172 IPC** + **94 .vue + 197 composable + 4 page (295 frontend)** + **88 Rust** 파일을 한 문서에 매핑. 신규 개발자 / 다음 세션 entry / dogfood 시점에 "어디까지 됐고 어디 남았나" 단일 진실원천.
>
...
## 3. 172 IPC 분포 (lib.rs 직접 카운트, c89+c95+ 후속 재집계)

> **c39 분해 후**: ipc/ 9 → 12 파일. 함수 수 자체는 170 보존 (re-export). c89+c95+ 누적 +2 → **172**. 카탈로그 자동 생성: [docs/api/tauri-commands.md](api/tauri-commands.md). 2026-05-18 doc-sync 갱신.
