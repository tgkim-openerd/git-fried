...
>
> **검증 SoT**: `apps/desktop/src-tauri/src/lib.rs` invoke_handler (167 IPC, 자동 카탈로그는 `bun scripts/generate-tauri-commands-index.mjs` → `docs/api/tauri-commands.md`), `apps/desktop/src-tauri/src/storage/migrations/` (**8 migration**, 0001~0008), CHANGELOG `[Unreleased]`, plan/13 + plan/14 + plan/16 + plan/22 + plan/25 의 자체 self-check 표.
...
| **테스트** | ✅ **vitest 90 files / 901 tests PASS** (c82) / cargo test **261 SoT / 245 PASS** (c89 Phase 1.4) / **E2E 11 spec / 50 tests PASS** (c47-mini) / bench compile | **SoT 명령**: `cargo test --no-run` 후 `target/debug/deps/{git_fried_lib,sqlite_pool_acquire_timeout}-*.exe --list \| grep -cE ": (test\|tokio::test)$"` (UltraPlan v0.4 A3 정립, 2026-05-16). **c89 Phase 1.1~1.4** cargo +20 — Phase 1.1 IPC fault injection + auth keyring (238→245) / Phase 1.2 Forge httpmock 401/403/429 (7 case) / Phase 1.3 worktree lifecycle 한글 path / Phase 1.4 SQLite pool acquire_timeout + tx auto-rollback. **c82** UltraPlan v0.1→v1.0 — perfMarks 84 LOC / vite 5.4→6.4.2 / SAF-401 child kill refactor + Semaphore + 1MB cap / vitest 89→90 / 901 PASS. **c42 cargo +2** (storage migration smoke). c41 cargo +13. c40 review-fix +10. c38~c41 누적. |
...
| **i18n 기초 인프라** | ✅ **활성화** (vue-i18n **11.4.2** + **ko/en 1343 leaf 완전 대칭 SoT** + JSON top-level dup-key raw regex 가드 정착) | **SoT 명령**: `node scripts/i18n-leaf-count.mjs` (출력 `ko=en=1343, symmetry OK`). **UltraPlan v0.4 sidebar Phase B (2026-05-16)**: `sidebar.resizeHandleAriaLabel/Title` (2) + `worktree.miniLockTitle/miniUnlockTitle` (2) 신규 = +4 (1339 → 1343). SB-001 resize handle + SB-009 worktree lock toggle. **UltraPlan v0.4 sidebar GitKraken DIFF (2026-05-16)**: `sidebar.quickTab.*` (10 key) + `sidebar.emptyState.*` (2 key) + `sidebar.footerStack` (1 key) 신규 = +13 (1326 → 1339). ActiveRepoQuickActions 5 quick tab 한글 하드코딩 + EmptyState 2 + Sidebar footer 1 마이그. **UltraPlan v0.4 LLM-test (2026-05-16)**: `a11y.ariaLabel.*` namespace 신규 (16 key × ko/en) — aria-label 한글 하드코딩 16건 회수 위한 SoT. 본 sprint 마이그 5건 (CommitDetailSidebar/CommitGraph/CommitMessageInput/settings) + 잔존 11건 (useI18n import 미사용 6 컴포넌트) 후속 sprint. 1311 → 1327 → 1326 (a11y.ariaLabel.close DRY 제거) → 1339 (+sidebar 13). **c89-B dup-key 회복** (commit `96d0be5` + 가드 `70e6ea9`): `gitkrakenImport` namespace ko/en 양쪽 중복 → 13 leaf silent drop, JSON.parse last-write-wins 메커니즘으로 i18n leaf count 우회. raw regex `^  \"([^\"]+)\":` 사후 검증 추가로 재발 차단. 누적 회복: 1298→1311 leaf. **c74** 폰트 pretendard → @fontsource Roboto Flex + Noto Sans KR + i18n 1201→1250. **c43~c48** 누적 +200 키. c40: vue-i18n ^9→^11. c38 +64 키. |
...
| `0006_repo_forge_account_override.sql` | c81 (2026-05-13) — per-repo forge account override (UltraPlan v0.5) |
| `0007_repo_ssh_key_override.sql` | c81 (2026-05-13) — per-repo SSH key override (UltraPlan v0.5) |
| `0008_keybindings_custom.sql` | 커스텀 단축키 사용자 override (Settings → Shortcuts) |

> Repository-Specific 설정은 별도 migration 없이 `.git/config` 직접 read/write (`git/config_local.rs`, plan/14 §3 결정 1). 단 forge account / SSH key override 는 0006/0007 별도 테이블 (DB 검색 + cross-repo audit 용).
