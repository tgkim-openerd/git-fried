...
const props = defineProps<{
  ...
  path: string | null
  open: boolean
  /** Sprint c30 / GitKraken UX (Phase 6a) — initial tab override (Blame 버튼 진입). */
...
const tab = ref<'history' | 'blame'>(props.initialTab ?? 'history')

// open 트랜지션 시 initialTab 재적용 (재오픈 케이스).
watch(
  () => props.open,
  (v) => {
...
<template>
  <BaseModal :open="isOpen" max-width="6xl" panel-class="h-[90vh]" @close="emit('close')">
    <template #header>
    ...
    <div v-if="tab === 'history'" class="flex flex-1 overflow-hidden">
      ...
      <div class="flex-1 overflow-auto p-4 text-sm">
        ...
        <div v-else>
          ...
          <pre class="whitespace-pre-wrap font-mono text-[12px]">{{
            selected.body || '(본문 없음)'
          }}</pre>
        </div>
      </div>
