        ...
        >
          <div class="flex items-center justify-between">
            <span class="font-mono text-xs">
              <span
                v-if="r.draft"
                class="rounded bg-amber-500/30 px-1 py-0.5 text-3xs text-warning-amber mr-1"
                >{{ t('releases.draftBadge') }}</span
              >
              <span
                v-if="r.prerelease"
                class="rounded bg-violet-500/30 px-1 py-0.5 text-3xs text-violet-700 dark:text-violet-500 mr-1"
                >{{ t('releases.preReleaseBadge') }}</span
              >
              {{ r.tag }}
            </span>
            <span class="text-3xs text-muted-foreground">{{ fmtDate(r.createdAt) }}</span>
          </div>
