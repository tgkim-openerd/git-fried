...
<template>
  <section data-testid="commit-detail-sidebar" class="flex h-full flex-col overflow-hidden bg-card">
    ...
    <!-- Sprint c77-C — 2단 layout: meta (subject/author/parents/stats) sticky, file list 만 scroll.
         ...
         c77-fix — <template v-else> 로 묶기 + border-b-2 border-border/60 시각 단서 강화. -->
    <template v-else>
      <div class="shrink-0 space-y-3 border-b-2 border-border/60 px-3 py-3">
      ...
      <!-- Sprint c77-C — 2단 layout 의 scrollable 영역: file list (Path/Tree toggle + ul).
           c77-fix — v-else 묶기 + Path/Tree toggle sticky-top (scroll 시 toggle 가시 유지). -->
      <div class="flex-1 space-y-3 overflow-auto px-3 py-3">
