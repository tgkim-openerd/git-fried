...
import { activateProfile, createProfile, deleteProfile, updateProfile } from '@/api/git'
...
async function onDelete(p: Profile) {
  const ok = await confirmDialog({
    ...
    message: t('confirm.deleteProfileMessage', { name: p.name }),
    danger: true,
  })
  if (!ok) return
...
async function onActivate(p: Profile) {
  if (p.isActive) return
  const ok = await confirmDialog({
    title: t('confirm.activateProfileTitle'),
    message: t('confirm.activateProfileMessage', { name: p.name }),
    danger: true,
  })
  if (!ok) return
