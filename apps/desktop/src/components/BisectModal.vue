  ...
  >
    <div class="p-4 text-sm">
      <p class="mb-3 text-xs text-muted-foreground">
        🔬 binary search 로 버그 commit 식별. 시작 → 현재 HEAD 가 좋은지 나쁜지 표시 → git 이
        자동으로 중간 commit 으로 checkout → 반복.
      </p>

      <div
        v-if="statusQuery.error.value"
        class="mb-3 rounded border border-destructive bg-destructive/10 p-2 text-xs"
      >
      ...
      <div v-else-if="statusQuery.data.value?.inProgress">
        <div class="mb-3 rounded-md border border-border bg-muted/20 p-3">
          ...
          <div class="mt-1 text-xs text-muted-foreground">
            Good: {{ statusQuery.data.value.good.length }} · Bad:
            {{ statusQuery.data.value.bad.length }}
          </div>
        </div>
        ...
        <div class="flex gap-2">
          <button
            type="button"
            class="rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-1.5 text-xs text-diff-add hover:bg-emerald-500/20 disabled:opacity-50"
          ...
          <button
            type="button"
            class="rounded-md border border-rose-500/40 bg-rose-500/10 px-3 py-1.5 text-xs text-danger-rose hover:bg-rose-500/20 disabled:opacity-50"
            :disabled="markMut.isPending.value"
