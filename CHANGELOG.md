...
- **Sprint c52 — branch chip column sticky-left + dev:cleanup helper + Rust toolchain mismatch 해소 (2026-05-07, 3 commits)** — c51 보류 #5 + checkpoint 다음 단계 #4 + c48~c51 잠복 1주일 toolchain 이슈 해소. vitest 65 → **66 files / 701 tests** (+1/+4) / cargo test **225 PASS 회복** (c48 부터 미실행 → c46+ 동일 수치) / toolkit solution `cargo-rustc-path-shadowing-by-chocolatey` 옵션 4 보강 (wrapper script + RUSTDOC env):
