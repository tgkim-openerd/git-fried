...
- **Sprint c52 — branch chip column sticky-left + dev:cleanup helper + Rust toolchain mismatch 해소 + TagPanel god comp 추출 (2026-05-07, 5 commits + toolkit 1)** — c51 보류 #5 + c48 보류 M1d + checkpoint 다음 단계 #1·#4 + c48~c51 잠복 1주일 toolchain 이슈 해소. vitest 65 → **67 files / 704 tests** (+2/+7) / cargo test **225 PASS 회복** (c48 부터 미실행 → c46+ 동일 수치) / i18n ko/en 954 → **964 leaf-keys** (+10 대칭) / toolkit solution `cargo-rustc-path-shadowing-by-chocolatey` 옵션 4 보강 (wrapper script + RUSTDOC env):
